module chordal_module
    ...
    function generate_nhats(settings,num_repeats,speeds) result(nhats)
        ...
        integer :: grade_nDims
        integer :: grade_index
        integer, dimension(size(settings%grade_index)+1) :: indices
        ...
        indices = [ settings%grade_index , settings%nDims ]

        ! Generate a sequence of random bases
        do i_grade=1,size(num_repeats)

            grade_nDims   = indices(i_grade+1)-indices(i_grade)+1
            grade_index   = settings%grade_index(i_grade)
