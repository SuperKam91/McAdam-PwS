...
module generate_module
    ...
    subroutine GenerateLivePoints(loglikelihood,settings,RTI,mpi_information)
        ...
        double precision,dimension(size(settings%grade_index)) :: speed
        ...
        if(is_root(mpi_information)) then
            ...
            if(.not. allocated(RTI%num_repeats) ) allocate(RTI%num_repeats(size(settings%grade_index)))
    ...
    subroutine time_speeds(loglikelihood,settings,RTI,speed,mpi_information)
        ...
        do i_speed=2,size(speed)
            ...
            h0 = settings%h0+settings%grade_index(i_speed)-1
            h1 = settings%h1
