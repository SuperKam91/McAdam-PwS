...
module feedback_module
    ...
    subroutine write_opening_statement(settings)
        ...
        if(settings%feedback >=normal_fb) then
            ...
            write(stdout_unit,'("nlive    :",I15)')   settings%nlive
            write(stdout_unit,'("nDims    :",I15)')   settings%nDims
            write(stdout_unit,'("nDerived :",I15)')   settings%nDerived
    ...
    subroutine write_started_generating(feedback)
        use utils_module,    only: stdout_unit,normal_fb,logzero
        implicit none
    ...
    subroutine write_intermediate_results(settings,RTI,nlikesum)
        ...
        if (settings%feedback>=normal_fb) then
            ...
            write(stdout_unit,'("ncluster   =",  I15," /",I15          )') RTI%ncluster, RTI%ncluster+RTI%ncluster_dead
            write(stdout_unit,'("ndead      =",  I15                   )') RTI%ndead
            write(stdout_unit,'("nposterior =",  I15                   )') RTI%nposterior_global(1)
            write(stdout_unit,'("nequals    =",  I15                   )') RTI%nequals_global(1)

            write(fmt_nlike,'("(""nlike      ="",",I0,"I15)")') size(nlikesum)
