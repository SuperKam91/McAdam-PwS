...
!------------------------------------------------------------------------
 ...
 ic_vnow,ic_npt,ic_nBrnch,ic_brnch,phyP,l,evDataAll,MassIndx,MassLim,MassMin,MassMax,IS,IS_Z,dumper,context)
 !subroutine pos_samp
  	implicit none
...
!		ic_infoloc(i1)=exp(d1-ic_zloc(i1))*ltmp(nPar+1)+exp(gZold-ic_zloc(i1))*(ic_infoloc(i1)+gZold)-ic_zloc(i1)
	...
	if(outfile) then
		...
		ic_infoloc(1:nClst), ic_nptloc(1:nClst),55,56,57,58,multimodal,MassIndx,MassLim,MassMin,MassMax)
		close(58)
...
!------------------------------------------------------------------------
  ...
  subroutine genSepFiles(npt,nPar,nCls,Ztol,pt,pwt,nCon,locZ,locInfo,locNpt,funit1,funit2,funit3,funit4,multimodal,MassIndx,MassLim,MassMin,MassMax)
	...
	double precision old_slocZ(3),sinfo(3),slocZ(3)
	character*30 fmt,stfmt

	nliveP=sum(locNpt(1:nCls))
	write(stfmt,'(a,i4,a)')  '(',nPar*4+4,'E28.18)'
	...
	do i=1,nCls
		...
		do j=k+1,k+nCon(i)
			!local evidence
			old_slocZ=slocZ
			slocZ(1)=logSumExp(slocZ(1),pt(nPar+1,j)+pt(nPar+2,j))
			!local info
			sinfo(1)=exp(pt(nPar+1,j)+pt(nPar+2,j)-slocZ(1))*pt(nPar+1,j)+exp(old_slocZ(1)-slocZ(1))*(sinfo(1)+old_slocZ(1))-slocZ(1)
			if(pt(MassIndx,j)<MassMin .or. pt(MassIndx,j)>MassMax) cycle
			if(pt(MassIndx,j)>=MassLim) then
				slocZ(3)=logSumExp(slocZ(3),pt(nPar+1,j)+pt(nPar+2,j))
				!local info
				sinfo(3)=exp(pt(nPar+1,j)+pt(nPar+2,j)-slocZ(3))*pt(nPar+1,j)+exp(old_slocZ(3)-slocZ(3))*(sinfo(3)+old_slocZ(3))-slocZ(3)
			else
				slocZ(2)=logSumExp(slocZ(2),pt(nPar+1,j)+pt(nPar+2,j))
				!local info
				sinfo(2)=exp(pt(nPar+1,j)+pt(nPar+2,j)-slocZ(2))*pt(nPar+1,j)+exp(old_slocZ(2)-slocZ(2))*(sinfo(2)+old_slocZ(2))-slocZ(2)
			endif
		enddo
...
!				endif
		...
		if(multimodal) then
			...
			d3=(nliveP-locNpt(i))*sinfo(1)/locInfo(i)+locNpt(i)
			write(funit3,'(a,E28.18,a,E28.18)')"Strictly Local Evidence",slocZ(1)," +/-",sqrt(sinfo(1)/locNpt(i))
		...
		write(funit3,'(a)')"Dim No.       Mean        Sigma"
      		...
      		write(funit3,'(a)')""
           	...
           	enddo
		write(funit4,stfmt)mean(i,1:nPar),sigma(i,1:nPar),maxLike(1:nPar),MAP(1:nPar),slocZ(1:3),d2
	enddo
