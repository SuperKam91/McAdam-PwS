...
module Nested
  ...
  logical :: IS = .true.
  integer MIndx
  double precision MLim,MMin,MMax
...
contains
  ...
  MassIndx,MassLim,MassMin,MassMax,loglike,dumper,context)
        
  	implicit none
        
	integer nest_ndims,nest_nlive,nest_updInt,context,seed,i
	integer maxClst,nest_nsc,nest_totPar,nest_nCdims,nest_pWrap(*),nest_maxIter,MassIndx
	logical nest_IS,nest_mmodal,nest_fb,nest_resume,nest_ceff,nest_outfile,initMPI
	character(LEN=100) nest_root
	double precision nest_tol,nest_ef,nest_Ztol,nest_logZero,MassLim,MassMin,MassMax
	...
	do ff=1,maxIter
		...
		if(ic_done(0)) then
			if(my_rank==0) then
				...
				l(1:nlive),evDataAll,MIndx,MLim,MMin,MMax,IS,IS_Z,dumper,context)
...
!					(ic_info(nd)+gZold)-ic_Z(nd)
			...
			if(my_rank==0) then
				if(sff>0 .and. (mod(sff,updInt)==0 .or. ic_done(0))) then
					...
					ic_nBrnch(1:ic_n),ic_brnch(1:ic_n,:,1),phyP(:,1:nlive),l(1:nlive),evDataAll,MIndx,MLim,MMin,MMax,IS,IS_Z,dumper,context)
				endif
			endif
