...
module read_write_module
    ...
    subroutine write_resume_file(settings,RTI)
        ...
        write(write_resume_unit,fmt_int) size(settings%grade_index)
        write(fmt_int,'("(",I0,A,")")') size(settings%grade_index),INT_FMT   ! define the integer array format
        write(write_resume_unit,'("=== positions of grades ===")')
        write(write_resume_unit,fmt_int) settings%grade_index
    ...
    subroutine read_resume_file(settings,RTI)
        ...
        read(read_resume_unit,*)                    
        read(read_resume_unit,fmt_int) i_temp
        if(size(settings%grade_index)/=i_temp) call halt_program('resume error: Grades do not match')
    ...
    function resume_file(settings,temp) result(file_name)
        ...
        if(temp) then
            ...
            file_name = trim(settings%base_dir) // '/' // trim(settings%file_root) // 'resume.dat'
        end if
    ...
    function stats_file(settings) result(file_name)
        ...
        file_name = trim(settings%base_dir) // '/' // trim(settings%file_root) // 'stats.dat'
    ...
    function posterior_file(settings,weighted,i) result(file_name)
        ...
        if(weighted) then
            ...
            file_name = trim(file_name) // 'post_equal_weights.dat'
        end if
    ...
    function phys_live_file(settings,i) result(file_name)
        ...
        if(present(i)) then
            ...
            file_name = trim(cluster_dir(settings)) // '/' // trim(settings%file_root) &
                // 'phys_live_' // trim(adjustl(cluster_num)) //'.points'
        else 
            !file_name = trim(settings%base_dir) // '/' // trim(settings%file_root) // '_phys_live.txt'
            file_name = trim(settings%base_dir) // '/' // trim(settings%file_root) // 'phys_live.points'
        end if
